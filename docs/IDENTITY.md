...
本地 Tauri 客户端登录后，当前浏览器会话会绑定到 loopback MCP 桥接。Agent 只需要连接 `http://127.0.0.1:3928/mcp`，不需要在 Agent 配置远程 Bearer 密钥。桥接只监听回环地址，退出登录、会话过期、成员撤权或切换空间后会拒绝后续调用。

这把本机操作系统用户视为可信边界：同一台机器上拥有该用户进程权限的程序理论上也能访问 loopback 端口。它阻断的是远程网络暴露和远程密钥泄漏；如果需要抵御同机恶意进程，应另外启用操作系统沙箱或使用显式 Agent 凭据。

如果 Agent 与客户端不在同一台机器，才在“账户与空间”的高级区域创建显式凭据。完整凭据只显示一次，绑定创建时所在的空间。以 `Authorization: Bearer <个人凭据>` 连接 `/mcp`，并通过受保护的网络通道传输。配置文件使用环境变量引用个人凭据，不提交真实 token。

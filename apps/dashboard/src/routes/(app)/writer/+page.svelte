...
<main class="writer-studio">
    ...
    <div class="studio-grid">
        <aside class="role-panel">
            <div class="section-heading"><h2>我的编剧角色</h2><button class="icon-button" onclick={() => showNewRole = !showNewRole} aria-label="新建编剧角色">＋</button></div>
            {#if showNewRole || (!roles.length && !loading)}
                <form class="new-role" onsubmit={e => { e.preventDefault(); void createRole(); }}>
                    ...
                    <button class="primary" type="submit" disabled={!!busy || !roleName.trim()}>创建角色</button>
                </form>
            {/if}
            ...
            {:else}
                ...
                <div class="tab-content">
                    {#if tab === 'sources'}
                        ...
                        <label class:disabled={!!busy} class="upload-zone"><input type="file" multiple accept=".txt,.md,.fountain,.fdx,.docx,.pdf" onchange={filesSelected} disabled={!!busy} /><Icon name="memories" size={30} /><strong>选择剧本文件，开始建立作品库</strong><span>先在本地解析，再交给你连接的 Agent 分析。扫描 PDF 请先 OCR。</span><b>上传作品</b></label>

<script lang="ts">
	...
	const FALLBACK_PROFILES: EmbeddingProfile[] = [
		{
			...
			description: zh("Vestige’s proven default. Kept intact while optional profiles are prepared beside it."),
		...
		},
		{
			...
			description: zh("Preview catalog profile. The local preflight reports its actual artifact and hardware requirements."),
	...
	let notice = $state(zh("Checking the local embedding service…"));
	...
	async function refresh() {
		refreshing = true;
		if (loading) notice = zh("Checking the local embedding service…");
		try {
			const state = await api.embeddings.profiles();
			applySnapshot(state);
			available = state.available ?? true;
			unavailableReason = null;
			notice = zh("Live local profile state loaded. Every vector space remains isolated.");
		} catch (cause) {
			available = false;
			...
			unavailableReason = zh("Embedding Profiles is not available from this Vestige server yet.");
			notice = zh("The service has not exposed Embedding Profiles yet. This preview cannot change models or download files.");
		} finally {
			loading = false;
			refreshing = false;
		}
	...
	function actionLabel(action: Action) {
		return zh(({ install: "Show install command", evaluate: "Show evaluation command", migrate: "Show migration command", activate: "Activate profile", rollback: "Roll back now" } as const)[action]);
	}
	...
	function actionDetail(action: Action, profile: EmbeddingProfile) {
		switch (action) {
			case 'install': return `校验并注册 ${zh(profile.name)} 的本地文件，暂不激活模型。`;
			case 'evaluate': return `先运行本地评测，再让 ${zh(profile.name)} 生成检索向量。`;
			case 'migrate': return `为 ${zh(profile.name)} 创建独立索引，保留现有 Nomic 向量。`;
			case 'activate': return `将检索原子切换到 ${zh(profile.name)}，保留先前配置以便回滚。`;
			case 'rollback': return `原子恢复 ${zh(profile.name)}，不删除文件或向量。`;
		}
	}

	function requestAction(action: Action, profile: EmbeddingProfile) {
		if (!available) {
			notice = zh("No action was sent: this server does not yet provide the Embedding Profiles API.");
			return;
		}
		if (requiresExplicitLocalCli(action)) {
			cliCommand = localCliCommand(action, profile);
			notice = `未从界面发送模型操作。请使用下方本地命令管理 ${zh(profile.name)}，模型目录不会经 HTTP 提交或保存。`;
			return;
		}
	...
	async function confirmAction() {
		...
		notice = `${actionLabel(action)} requested for ${zh(profile.name)}. Waiting for the local receipt…`;
		try {
			...
			if (isUnavailable(cause)) {
				available = false;
				unavailableReason = zh("Embedding Profiles is not available from this Vestige server yet.");
				notice = zh("No model state changed: the Embedding Profiles API is unavailable on this server.");
			} else {
				notice = zh("The local operation did not complete. Review the local server receipt before trying again.");
			}
		} finally {
			busy = null;
		}
	...
	function stageLabel(profile: EmbeddingProfile) {
		...
		if (profile.migration?.state === 'validating') return zh("Validating index");
		if (profile.migration?.state === 'paused') return zh("Migration paused");
		if (profile.migration?.state === 'cancelled') return zh("Migration cancelled");
		if (profile.evaluation?.state === 'running') return 'Evaluating';
		return profile.stage.replaceAll('_', ' ');
	}

	function formatBytes(value?: number) {
		if (value == null) return zh("reported at preflight");
		if (value === 0) return zh("already bundled");
...
<svelte:head><title>{zh("Embedding Profiles · Vestige")}</title></svelte:head>

<main class="embeddings-shell" style="position: relative">
	<AmbientField {...ambient} accent={[0.55, 0.78, 0.86]} opacity={0.5} />
	<PageHeader icon="embeddings" accent="synapse" title={zh("Embedding Profiles")} subtitle={zh("Local vector spaces with a deliberate, receipt-backed path from install to rollback.")}>
		<button class="refresh" type="button" onclick={refresh} disabled={refreshing || busy !== null}>
			{refreshing ? zh("Refreshing…") : zh("Refresh local state")}
		</button>
	</PageHeader>

	<section class:unavailable={available === false} class="local-contract" aria-label={zh("Embedding profile safety contract")}>
		<div class="contract-icon"><Icon name={available === false ? 'pulse' : 'embeddings'} size={20} /></div>
		<div><strong>{available === false ? zh("Preview only — service unavailable") : zh("Local-only, profile-isolated retrieval")}</strong><p>{available === false ? zh("No controls can download a model, switch retrieval, or re-embed memories until the server advertises these endpoints.") : zh("Install, evaluation, and migration run through an explicit local CLI command with a verified artifact directory. This dashboard only reads receipts and performs separately confirmed, fully-gated activation or rollback. Vestige never compares vectors across profiles.")}</p></div>
	</section>

	{#if unavailableReason}
		<p class="availability-note" role="status"><span>{zh("LOCAL API")}</span>{unavailableReason}</p>
	{/if}

	<section class="active-strip" aria-label={zh("Current embedding profile")}>
		<div><p class="eyebrow">{zh("ACTIVE RETRIEVAL POINTER")}</p><strong>{zh(activeProfile?.name ?? zh("No profile active"))}</strong><span>{activeProfile?.modelId ?? zh("No local vector space selected")}</span></div>
		<div class="pointer-rule"><i></i><span>{zh("one active profile")}</span><i></i></div>
		<div class="rollback"><p class="eyebrow">{zh("ROLLBACK")}</p>{#if rollbackProfile}<strong>{zh(rollbackProfile.name)}</strong><button type="button" onclick={() => requestAction('rollback', rollbackProfile)} disabled={busy !== null || available === false}>{zh("Restore preserved profile")}</button>{:else}<strong>{zh("Nothing pending")}</strong><span>{zh("The current profile has no newer activation to undo.")}</span>{/if}</div>
	</section>
	...
	<section class="profile-grid" aria-busy={loading}>
		{#each profiles as profile (profile.id)}
			...
			<article class:active={profile.active} class="profile-card">
				<header>
					<div><p class="eyebrow">{profile.active ? zh("DEFAULT · PRESERVED") : zh("OPTIONAL PROFILE")}</p><h2>{zh(profile.name)}</h2><code>{profile.modelId}</code></div>
					<span class:active={profile.active} class="stage">{zh(stageLabel(profile))}</span>
				</header>
				<p class="description">{zh(profile.description ?? zh("A local profile with its own encoder and vector contract."))}</p>

				<dl class="tradeoffs">
					<div><dt>{zh("Encoder")}</dt><dd>{profile.dimensions.toLocaleString('zh-CN')} {zh("dimensions")}</dd></div>
					<div><dt>{zh("Context")}</dt><dd>{profile.maxTokens?.toLocaleString('zh-CN') ?? '—'} {zh("tokens")}</dd></div>
					<div><dt>{zh("Model disk")}</dt><dd>{formatBytes(profile.modelBytes)}</dd></div>
					<div><dt>{zh("Vector / memory")}</dt><dd>{formatBytes(profile.vectorBytes)}</dd></div>
					<div class="wide"><dt>{zh("Hardware path")}</dt><dd>{zh(profile.hardware ?? zh("Local hardware preflight required"))}</dd></div>
				</dl>

				<div class="stages" aria-label={`${zh(profile.name)} staged workflow`}>
					<span class:done={profile.installed}>{zh("1 Install")}</span><i></i>
					<span class:done={profile.evaluation?.state === 'complete'}>{zh("2 Evaluate")}</span><i></i>
					<span class:done={['complete', 'not_required'].includes(profile.migration?.state ?? '')}>{zh("3 Isolate vectors")}</span><i></i>
					<span class:done={profile.active}>{zh("4 Activate")}</span>
				</div>

				{#if percent !== null}
					<div class="migration-progress"><div><span>{zh("Separate index migration")}</span><strong>{profile.migration?.completed?.toLocaleString('zh-CN')} / {profile.migration?.total?.toLocaleString('zh-CN')}</strong></div><progress value={percent} max="100">{percent}%</progress></div>
				{/if}
				{#if profile.evaluation?.state === 'complete'}
					<p class="evaluation"><Icon name="pulse" size={14} /> {profile.evaluation.metric ?? zh("Local evaluation complete")}{#if profile.evaluation.score != null}: {profile.evaluation.score.toFixed(3)}{/if}{#if profile.evaluation.sampleSize != null} · {profile.evaluation.sampleSize.toLocaleString('zh-CN')} {zh("cases")}{/if}</p>
				{/if}

				<footer>
					{#if profile.lastReceipt?.summary}<p class="receipt"><span>{zh("RECEIPT")}</span>{zh(profile.lastReceipt.summary)}</p>{:else}<p class="receipt"><span>{zh("CONTRACT")}</span>{profile.active ? zh("Default is unchanged until you explicitly activate another evaluated profile.") : zh("No changes occur until the next explicit stage is confirmed.")}</p>{/if}
					{#if next}<button class="action" type="button" disabled={busy !== null || available === false} onclick={() => requestAction(next, profile)}>{busy?.profileId === profile.id ? `${zh(stageLabel(profile))}…` : actionLabel(next)} <span>→</span></button>{:else if profile.active}<span class="active-state">{zh("● Retrieval is using this profile")}</span>{/if}
				</footer>
			</article>
		{/each}
	</section>

	<section class="operation-status" aria-live="polite"><p class="eyebrow">{zh("OPERATION STATUS")}</p><output>{notice}</output>{#if cliCommand}<p class="cli-instruction"><span>{zh("RUN LOCALLY AGAINST THIS VESTIGE DATA DIRECTORY")}</span><code>{cliCommand}</code><small>{zh("Replace the placeholder with the verified local artifact directory. This page never sends that path to the server.")}</small></p>{/if}</section>
</main>
...
{#if pending}
	<div class="confirmation-backdrop" role="presentation">
		<div class="confirmation" role="dialog" aria-modal="true" aria-labelledby="confirm-title">
			<p class="eyebrow">{zh("EXPLICIT CONFIRMATION")}</p><h2 id="confirm-title">{actionLabel(pending.action)}?</h2><p>{actionDetail(pending.action, pending.profile)}</p>
			<div class="confirmation-contract"><Icon name="embeddings" size={17} /><span>{zh("Local only. This stage does not silently run any later stage.")}</span></div>
			<div class="confirmation-actions"><button type="button" class="cancel" onclick={() => (pending = null)}>{zh("Cancel")}</button><button type="button" class="confirm" onclick={confirmAction}>{zh("Confirm")} {actionLabel(pending.action)}</button></div>
		</div>
	</div>
{/if}
